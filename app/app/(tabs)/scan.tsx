...
function imageToScreen(
  ...
  return { x: (viewW - dispW) / 2 + nx * dispW, y: (viewH - dispH) / 2 + ny * dispH };
}

type Pt = { x: number; y: number };

function bbox(pts: Pt[]): { left: number; top: number; width: number; height: number } {
...
function CardDetectionOverlay({
  ...
  const proj = (nx: number, ny: number) => imageToScreen(nx, ny, imageW, imageH, viewW, viewH, cover);
  ...
  const rightVec = cardW > 0 ? { x: (br.x - bl.x) / cardW, y: (br.y - bl.y) / cardW } : { x: 1, y: 0 };
  const upVec    = cardH > 0 ? { x: (tl.x - bl.x) / cardH, y: (tl.y - bl.y) / cardH } : { x: 0, y: -1 };
  ...
  const nameDownVec = cardH > 0 ? { x: (bl.x - tl.x) / cardH, y: (bl.y - tl.y) / cardH } : { x: 0, y: 1 };
  ...
  const quadBB = bbox([tl, tr, br, bl]);
  const blBB   = bbox([ocrTL, ocrTR, ocrBR, ocrBL]);
  const nameBB = bbox([nameTL, nameTR, nameBR, nameBL]);

  return (
    <>
      {/* Detected card — axis-aligned bounding box of the detected quad */}
      <View pointerEvents="none" style={[overlayBoxBase, quadBB, { borderWidth: 3, borderColor: NAME_REGION_COLOR }]} />
      {/* Bottom-left OCR region */}
      <View pointerEvents="none" style={[overlayBoxBase, blBB, { borderWidth: 2, borderColor: blColor }]} />
      {/* Name OCR region */}
      <View pointerEvents="none" style={[overlayBoxBase, nameBB, { borderWidth: 2, borderColor: nameColor }]} />
      ...
      {blText != null && (
        ...
        >
          <Text style={[cropLabelTitle, { color: BL_REGION_COLOR }]}>BL CROP</Text>
          <Text style={cropLabelBody}>{blText || '(empty)'}</Text>
        </View>
      )}
      ...
      {ocrText != null && ocrText !== blText && (
        ...
        >
          <Text style={[cropLabelTitle, { color: NAME_REGION_COLOR }]}>NAME CROP</Text>
          <Text style={cropLabelBody}>{ocrText}</Text>
        </View>
      )}
    </>
  );
...
const overlayBoxBase = { position: 'absolute' as const };
const cropLabelBase = {
...
const cropLabelTitle = { fontSize: 9, fontWeight: '700' as const, letterSpacing: 0.8, marginBottom: 2 };
const cropLabelBody = { color: 'rgba(255,255,255,0.85)', fontSize: 10, fontFamily: 'monospace', lineHeight: 14 };
...
function OcrDebugPanel({
  ...
  const strategyLabel = strategy === 'set_number' ? 'SET + NUMBER'
    : strategy === 'name' ? 'NAME FALLBACK'
    : null;

  let statusLabel: string | null = null;
  switch (phase.status) {
    case 'scanning': statusLabel = 'Detecting card…'; break;
    case 'fetching': statusLabel = 'Looking up card…'; break;
    case 'error':    statusLabel = phase.message; break;
  }

  return (
    <View style={debugStyles.panel}>
      <Text style={debugStyles.header}>OCR DEBUG</Text>
      {strategyLabel && <Text style={debugStyles.strategy}>{strategyLabel}</Text>}
      {statusLabel && (
        <Text style={[debugStyles.status, phase.status === 'error' ? debugStyles.statusError : debugStyles.statusOk]}>
          {statusLabel}
        </Text>
      )}
      {blText != null && <Text style={debugStyles.ocrLine}>BL: {blText.replace(/\n/g, ' | ') || '(empty)'}</Text>}
      {parsed && <Text style={debugStyles.ocrLine}>parsed: {parsed.setCode.toUpperCase()} #{parsed.collectorNumber}</Text>}
      {ocrText != null && ocrText !== blText && (
        <Text style={debugStyles.ocrLine}>NAME: {ocrText.replace(/\n/g, ' | ') || '(empty)'}</Text>
      )}
      {queryInfo && <Text style={debugStyles.ocrLine}>→ {queryInfo}</Text>}
    </View>
  );
}
...
export default function ScanScreen() {
  ...
  const [overlayLayout, setOverlayLayout] = useState(() => ({ width: winW, height: winH }));
  ...
  useEffect(() => { setCardPlugin(initCardDetectorPlugin()); }, []);
  useEffect(() => () => { clearSessionCardCache(); }, []);
  ...
  const openPanel = useCallback(() => {
    setPanelOpen(true);
    Animated.timing(panelHeightAnim, { toValue: PANEL_MAX_HEIGHT, duration: 250, useNativeDriver: false }).start();
  }, [panelHeightAnim]);

  const closePanel = useCallback(() => {
    Animated.timing(panelHeightAnim, { toValue: 0, duration: 200, useNativeDriver: false })
  ...
  const triggerOcr = useCallback(async () => {
    ...
    try {
      ...
      const result = await scanCard(uri, handleProgress, { width: photo.width, height: photo.height }, corners);
  ...
  }, [addRecentScan, setLastScannedId, isCapturing, handleProgress]);

  const jsSetDetection = useRunOnJS((d: DetectionInfo | null) => { setDetection(d); }, [setDetection]);
  ...
  const pickFromLibrary = useCallback(async () => {
    ...
    try {
      ...
      // Fall through to OCR.
      const result = await scanCard(asset.uri, handleProgress, undefined, corners);
      upsertCard(result.card);
      addRecentScan(result.card);
      setLastScannedId(result.card.scryfall_id);
      setScanStrategy(result.strategy);
  ...
  }, [addRecentScan, setLastScannedId, isCapturing, smoothedCornersWv, handleProgress]);

  const handleOverlayLayout = useCallback((e: LayoutChangeEvent) => {
    setOverlayLayout(e.nativeEvent.layout);
  }, []);
  ...
  let statusLabel: string | null = null;
  switch (phase.status) {
    case 'idle':     statusLabel = 'Point camera at a card'; break;
    case 'scanning': statusLabel = 'Detecting card…'; break;
    case 'fetching': statusLabel = 'Looking up card…'; break;
    case 'error':    statusLabel = null; break; // shown as toast instead
  }

  const buildOverlay = (cover: boolean) => (
    <View style={styles.overlay} pointerEvents="none" onLayout={handleOverlayLayout}>
      {detection && (
  ...
  return (
    <View style={styles.screen}>
      ...
      {phase.status === 'error' && <ErrorToast message={phase.message} />}
      ...
      <Animated.View
        style={[
          scanStyles.panel,
          {
            ...
            opacity: panelHeightAnim.interpolate({ inputRange: [0, 1], outputRange: [0, 1], extrapolate: 'clamp' }),
          },
        ]}
...
const styles = StyleSheet.create({
  ...
  pill: { paddingHorizontal: 28, paddingVertical: 12, borderRadius: 100, alignItems: 'center', justifyContent: 'center' },
  pillSecondary: { backgroundColor: 'rgba(255,255,255,0.08)', borderWidth: StyleSheet.hairlineWidth },
  ...
  camera: { flex: 1 },
  fullScreenImageContainer: { ...StyleSheet.absoluteFillObject, zIndex: 10, backgroundColor: '#000' },
  ...
  errorToastText: { color: '#fff', fontSize: 14, fontWeight: '500', lineHeight: 20, textAlign: 'center' },
  ...
  footer: { padding: 24, paddingBottom: 32, alignItems: 'center', gap: 12, zIndex: 20 },
...
const debugStyles = StyleSheet.create({
  ...
  strategy: {
    color: '#4ecdc4', fontSize: 11, fontWeight: '700',
    fontFamily: 'monospace', marginBottom: 4, letterSpacing: 0.8,
  },
  status: { fontSize: 11, fontFamily: 'monospace' },
  statusOk: { color: 'rgba(255,255,255,0.6)' },
  statusError: { color: '#ef5350', fontWeight: '600' },
  ocrLine: { color: 'rgba(255,255,255,0.8)', fontSize: 10, fontFamily: 'monospace', marginTop: 3 },
});

const scanStyles = StyleSheet.create({
  successBadge: {
    position: 'absolute',
    top: 12, alignSelf: 'center', zIndex: 100,
  ...
  successText: { color: '#fff', fontSize: 14, fontWeight: '600', letterSpacing: 0.2 },
  recentBtn: {
    position: 'absolute', right: spacing.lg, zIndex: 50,
  ...
  recentBtnDisabled: { opacity: 0.35 },
  recentBadge: {
  ...
  recentBadgeText: { color: '#000', fontSize: 10, fontWeight: '700' },
  panel: {
    position: 'absolute', left: spacing.md, right: spacing.md, zIndex: 49,
  ...
  thumb: { width: 48, height: 68, borderRadius: 4, backgroundColor: 'rgba(255,255,255,0.08)' },
  rowInfo: { flex: 1, gap: 3 },
  rowName: { color: '#fff', fontSize: 14, fontWeight: '600' },
  rowSet: { color: 'rgba(255,255,255,0.5)', fontSize: 11 },
  rowPrice: { color: '#4ecdc4', fontSize: 13, fontWeight: '500', minWidth: 44, textAlign: 'right' },
  separator: { height: StyleSheet.hairlineWidth, backgroundColor: 'rgba(255,255,255,0.1)', marginLeft: 72 },
});
